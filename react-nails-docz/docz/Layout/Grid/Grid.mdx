...
const { SpacingType, TextAlignType, SizeType } = Types;
...
All Properties can be overriden by containing Grid.Row and Grid.Column properties to explicitly set properties for certain grid parts.
...
<Box>
    {Object.entries(SpacingType).map(([name, space]) => (
        <Box key={name}>
            <ThemeProps>
                {theme => (
                    <HeaderText size={SizeType.Small}>
